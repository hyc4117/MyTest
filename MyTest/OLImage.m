...
+ (id)imageWithData:(NSData *)data scale:(CGFloat)scale
{
    ...
    if (imageSource && CGImageSourceContainsAnimatedGif(imageSource)) {
...
{
	[self updateWithData:data final:NO]; //改为YES。imageWithIncrementalData可以重复播放gif
}

- (void)updateWithData:(NSData *)data final:(BOOL)final
{
    if (![self isPartial] || !data || data.length == 0) {
        NSLog(@"gif update data zero");
        return;
    }
...
- (void)updateDurations
{
	...
	if (count > 0){
		durations = calloc(count, sizeof(NSTimeInterval));
...
- (id)_objectAtIndex:(NSUInteger)idx
{
    ...
    if (!frameImageRef)
    {
        NSLog(@"read gif image Fail!");
        return nil;
    }
    UIImage *image = [UIImage imageWithCGImage:frameImageRef
                                         scale:self.scale
